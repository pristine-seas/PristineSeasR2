make_stations <- function() {
  tibble::tibble(
    ...
    ps_site_id           = c("A", "A", "B"),
    divers               = c("Molly Timmers", "Molly Timmers", "Molly Timmers"),
    depth_strata         = c("shallow", "deep", "shallow"),
    depth_m              = c(10, 20, 10),
    n_taxa               = c(12, 8, 15),
    avg_density_m2       = c(0.5, 0.3, 0.8),
    avg_density_m2_clams = c(0.05, 0.02, 0.10),
    avg_density_m2_cucs  = c(0.01, 0.00, 0.03)
  )
}
...
test_that("returns a leaflet htmlwidget for valid input", {
  m <- explore_invert_density(make_stations(), make_sites())
  expect_s3_class(m, "leaflet")
  expect_s3_class(m, "htmlwidget")
})

test_that("errors when stations is missing a required column", {
  bad <- dplyr::select(make_stations(), -avg_density_m2_clams)
  expect_error(explore_invert_density(bad, make_sites()), "avg_density_m2_clams")
})

test_that("errors when sites is missing a required column", {
  bad <- dplyr::select(make_sites(), -exposure)
  expect_error(explore_invert_density(make_stations(), bad), "exposure")
})

test_that("errors when export_path is given without a title", {
  expect_error(
    explore_invert_density(make_stations(), make_sites(),
...
test_that("errors when a site in stations has no matching coordinates", {
  ...
  expect_error(explore_invert_density(stations, sites), "coordinates")
})
...
test_that("writes a standalone HTML file when export_path and title are given", {
  ...
  m <- explore_invert_density(make_stations(), make_sites(),
                              title       = "Test Expedition — Invertebrate survey results",
                              export_path = tmp)
...
test_that("works with a single site", {
  ...
  sites    <- make_sites()[1, ]
  expect_no_error(explore_invert_density(stations, sites))
})
...
test_that("works when multiple stations share the same site", {
  ...
  expect_no_error(explore_invert_density(make_stations(), make_sites()))
})
