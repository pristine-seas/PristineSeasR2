# explore_fish_biomass.R -------------------------------------------------------
# Interactive per-site fish survey results map (metric-toggle circle markers)
#
# Public API:
#   - explore_fish_biomass(): builds the leaflet widget used across UVS/BLT
#     fish reports — one circle marker per site, with both size and color
#     driven by whichever metric (taxa richness / density / biomass) is
#     toggled via the layer control, so it looks identical expedition to
#     expedition.
#
...
# map_utils.R, same as explore_uvs_sites() and explore_benthic_cover().

#' Build the Interactive Fish Survey Results Map (Metric Toggle by Site)
#'
#' @description
#' Builds the standard Pristine Seas per-site fish survey map: station-level
#' taxa richness, density, and biomass are averaged up to one circle marker
#' per site, with both marker size and color driven by whichever metric is
#' active. A layer control (top right) toggles between the three metrics,
#' swapping the color ramp and the gradient legend (bottom right) to match.
#' This is the same map used across UVS/BLT fish reports, so calling it
...
#'   `station_summary`. Must include `ps_site_id`, `divers`, `depth_strata`,
#'   `depth_m`, `n_taxa`, `avg_density_m2`, and `avg_biomass_gm2`.
...
#' @export
explore_fish_biomass <- function(stations,
                                  sites,
                                  title       = NULL,
  ...
  required_station_cols <- c("ps_site_id", "divers", "depth_strata", "depth_m",
                              "n_taxa", "avg_density_m2", "avg_biomass_gm2")
  ...
  by_site <- stations |>
    ...
    dplyr::summarise(
      ...
      avg_biomass_gm2 = mean(.data$avg_biomass_gm2, na.rm = TRUE),
      .groups = "drop"
    ) |>
    dplyr::left_join(
  ...
  metrics <- list(
    ...
    list(key = "Biomass",       col = "avg_biomass_gm2", unit = "g / m²",
         digits = 1, pal = viridisLite::mako(256))
  )
  ...
  popups <- vapply(seq_len(nrow(by_site)), function(i) {
    ...
    mblock <- paste0(
      ...
      metric_row("Biomass",       fmt_num(by_site$avg_biomass_gm2[i], 1), "g/m²"),
      "</div>")
